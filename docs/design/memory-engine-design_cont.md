# AegisOps AI — Persistent Operational Memory Engine Design (Updated)
...
AegisOps/
  ...
  tests/
    ...
    test_memory_integration_postgres.py
    conftest.py
  docs/
    memory-engine.md
    decisions/
...
This structure intentionally keeps the future Neo4j/Qdrant adapters out of Phase 1. The only integration layer present is a protocol/interface definition, not implementation code.
...
- new memory records are created with `status = active`
- only the service layer can create lineage via supersession
...
Database constraints remain authoritative for `confidence` and `importance` values even when service or Pydantic validation is applied. This is important because invalid values must fail even if application logic is bypassed or if request payloads are malformed.
...
- use row-level locking for the existing record

### Risk: lifecycle misuse through PATCH

Mitigation:

- `supersedes_memory_id` is not accepted in the patch schema
- supersession lineage is only created using the dedicated service method
...
11. only the `MemoryService.supersede_memory()` operation may create lineage
12. lifecycle transitions beyond the approved set are rejected unless explicitly added later

---

## Summary

This updated design establishes a strict and traceable canonical memory layer for AegisOps AI without building the later graph/vector/agent stack. PostgreSQL owns the authoritative record state, while the future Neo4j and Qdrant adapters will provide relationship-aware and semantic representations as derived views. The lifecycle is intentionally versioned and non-destructive so operational memory remains auditable, explainable, and safe for future enterprise decision support.
